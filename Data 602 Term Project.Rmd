...
```{r}
#plotting the confidence intervals
ggplot(room_type_means, aes(x = reorder(room_type, mean_price), y = mean_price)) + geom_point() + geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.2) + coord_flip() + labs(x = "Room Type", y = "Mean Price", title = "Mean Price by Room Type with Confidence Intervals")
```
...
```{r}
anova_result <- aov(price ~ neighbourhood_cleansed + room_type, data = listings)

summary(anova_result)

# Perform ANOVA
#anova_result <- aov(mean_prices ~ neighborhoods, data = data)
